...
def reqAppt_calendar(request):
    ...
    def previous_month(dday):
        ...
        calMonth = 'month=' + str(previous_month.year) + \
            '-' + str(previous_month.month)
        return calMonth
...
def create_Appointment(request):
    if request.method == 'POST':
        ...
        if not form.is_valid():
            ...
            print(apptDate)
            print(apptHour)
            meetingDate = timezone.make_aware(
                datetime.datetime.strptime(apptDate, "%m/%d/%Y"))
...
def Doctor_view(request):
    x = ApptTable.objects.filter(
        provider=request.user.provider.id).order_by('meetingDate')
...
def Patient_view(request):
    x = ApptTable.objects.filter(
        patient=request.user.patient.id).order_by('meetingDate')
    pagination = Paginator(x, 5)
    page = request.GET.get('page', 1)
    try:
        pagination = pagination.page(page)
...
def Doctor_avail_view(request, id, date_str):
    ...
    print(ApptTable.objects.filter(
        provider_id=id,
        meetingDate__year=str(year),
        meetingDate__month=str(month),
        meetingDate__day=str(day),
    ...
    data = [h for h in range(STARTING_HOUR, ENDING_HOUR + 1)
            if h not in unavailable_times]
...
def event(request):
    ...
    is_patient = [type_name for t, type_name in USER_TYPE_CHOICES if t ==
                  request.user.user_type][0] == 'Patient'
    if is_patient:
        all_meetings = ApptTable.objects.filter(
            patient__user_id=request.user.id).all()
    else:
        all_meetings = ApptTable.objects.filter(
            provider__user_id=request.user.id).all()
    ...
    for i in all_meetings:
        ...
        #so this gets the utc timezone aware object of the saved record in the database so we need to convert it to the current time zone 
        temp =timezone.make_aware(timezone.make_naive(i.meetingDate), timezone=timezone.get_current_timezone())
        start = temp.strftime('%Y-%m-%dT%H:%M:%S')
        end = (temp + timedelta(hours=1)
